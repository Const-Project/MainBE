...
class UserDailyMissionServiceTest {
        ...
        void getDailyMissions_Success() {
            // Given (준비)
            Long userId = 1L;
            DailyMissionMasters missionMaster1 = DailyMissionMasters.builder().id(101L).title("미션 1").build();
            DailyMissionMasters missionMaster2 = DailyMissionMasters.builder().id(102L).title("미션 2").build();

            UserDailyMissions userMission1 = UserDailyMissions.builder().id(1L).dailyMissionMasters(missionMaster1).build();
            UserDailyMissions userMission2 = UserDailyMissions.builder().id(2L).dailyMissionMasters(missionMaster2).build();
        ...
        void uploadPictureForDailyMission_Success() {
            ...
            // When
            String imageUrl = userDailyMissionService.uploadPictureForDailyMission(userDailyMissionId, mockFile);
        ...
        void summitAnswer_Correct() {
            ...
            DailyMissionMasters missionMaster = DailyMissionMasters.builder().id(dailyMissionMasterId).build();
            UserDailyMissions userMission = UserDailyMissions.builder().id(userDailyMissionId).dailyMissionMasters(missionMaster).build();
            Quiz quiz = Quiz.builder().id(quizId).dailyMissionMasters(missionMaster).build();

            List<QuizOptions> options = List.of(
            ...
            given(userDailyMissionRepository.findById(userDailyMissionId)).willReturn(Optional.of(userMission));
            given(quizRepository.findByDailyMissionMasters_Id(dailyMissionMasterId)).willReturn(Optional.of(quiz));
        ...
        void summitAnswer_Fail_QuizNotFound() {
            ...
            given(userDailyMissionRepository.findById(userDailyMissionId)).willReturn(Optional.of(userMission));
            given(quizRepository.findByDailyMissionMasters_Id(dailyMissionMasterId)).willReturn(Optional.empty());
