...
class QuizServiceTest {
    ...
    void getQuiz_Success() {
        ...
        DailyMissionMasters dailyMissionMasters = DailyMissionMasters.builder()
                .id(dailyMissionMasterId)
                .title("오늘의 퀴즈 미션")
                .build();

        UserDailyMissions userDailyMissions = UserDailyMissions.builder()
        ...
        List<QuizOptions> quizOptions = List.of(
        ...
        // Mock 객체 동작 정의
        given(userDailyMissionRepository.findById(userDailyMissionId)).willReturn(Optional.of(userDailyMissions));
    ...
    void getQuiz_Fail_QuizNotFound() {
        ...
        DailyMissionMasters dailyMissionMasters = DailyMissionMasters.builder().id(dailyMissionMasterId).build();
        UserDailyMissions userDailyMissions = UserDailyMissions.builder().id(userDailyMissionId).dailyMissionMasters(dailyMissionMasters).build();

        given(userDailyMissionRepository.findById(userDailyMissionId)).willReturn(Optional.of(userDailyMissions));
        given(quizRepository.findByDailyMissionMasters_Id(dailyMissionMasterId)).willReturn(Optional.empty()); // 퀴즈가 없음
