package com.example.cp_main_be.domain.mission.user_daily_mission.domain.repository;
...
public interface UserDailyMissionRepository extends JpaRepository<UserDailyMission, Long> {
  ...
  @Query(
      "SELECT udm FROM UserDailyMission udm "
          + "JOIN FETCH udm.dailyMissionMaster "
          + "WHERE udm.user = :user AND udm.createdAt BETWEEN :startDate AND :endDate")
  List<UserDailyMission> findTodayMissionsWithMasterByUser(
      @Param("user") User user,
      @Param("startDate") LocalDateTime startDate,
      @Param("endDate") LocalDateTime endDate);

  List<UserDailyMission> findByUserIdAndCreatedAtAfter(Long userId, LocalDateTime dateTime);

  @Modifying
  @Query("DELETE FROM UserDailyMission udm WHERE udm.user = :user")
